...
public class MyArrayList<E> 
{
 ...
 public boolean sortList() {
  ...
  public void insertSorted(E e) { //method of type E (e - number you want to insert on the array)
	int i;
	
	for(i = size - 1; i >= 0; i--) 		//must start from the back because the array is sorted in ascending order; end in the start of the array and it must decrement
		{
			if(((Comparable)data[i]).compareTo(e)>0)
				{
					data[i+1] = data[i];		//shift the elements to the right
					data[i]=e;					//overritting the value of the current element with the number that you want to insert (e)
				}
		}
    size++;	  //if an element is sorted the size of the array increases
  }
